...
if("${HOST_TARGET_TRIPLE}" STREQUAL "x86_64-pc-windows-msvc")
    set(artifact_relpath "libs/amd64/eta_http.dll")
elseif("${HOST_TARGET_TRIPLE}" STREQUAL "x86_64-unknown-linux-gnu")
    set(artifact_relpath "libs/amd64/libeta_http.so")
elseif("${HOST_TARGET_TRIPLE}" STREQUAL "aarch64-unknown-linux-gnu")
    set(artifact_relpath "libs/arm64/libeta_http.so")
elseif("${HOST_TARGET_TRIPLE}" STREQUAL "x86_64-apple-darwin")
    set(artifact_relpath "libs/amd64/libeta_http.dylib")
elseif("${HOST_TARGET_TRIPLE}" STREQUAL "aarch64-apple-darwin")
    set(artifact_relpath "libs/arm64/libeta_http.dylib")
else()
    message(STATUS
        "eta-http staging: skipping unsupported host triple ${HOST_TARGET_TRIPLE}")
    return()
endif()

# The sidecar MODULE is built directly into libs/${arch}/ via
...
if(updated_sha)
    ...
    message(STATUS
        "eta-http staging: updated sha256 for ${HOST_TARGET_TRIPLE}: ${artifact_sha256}")
else()
    message(STATUS
        "eta-http staging: no sha256 entry for ${HOST_TARGET_TRIPLE} in ${manifest_path}; skipping update")
endif()
