...
| **Distribution** | A built artifact — a tarball or directory — produced by `eta build` (default `release` profile) for installation or upload. Contains `.etac` files and a frozen manifest. |
...
### 4.2.1 Repository example package layout

For this repository, keep one canonical packaging fixture at
`packages/example/hello-world/` so CI and docs share the same smoke
target:
...
`eta build`, `eta run`, and `eta test` on this fixture are part of the
...
# === Build settings ===
[build]
default-profile = "release"   # "release" | "debug"
optimize  = true              # maps to `etac -O`
strip     = true              # maps to `etac --no-debug`
emit-stats = false

[build.debug]        # used by debugger / editor flows
optimize  = false
strip     = false

[build.release]      # explicit production profile (same as [build] defaults)
optimize  = true
strip     = true
...
- `[dev-dependencies]` only built for `eta test` / `eta bench`; not in
  the dep graph of `eta build` (default `release` profile).
...
| `eta run [--profile <release\|debug>] [--bin NAME] [--example NAME] [-- args…]` | S2 | (manifest optional) | Manifest mode: runs with resolved module path; default profile is `release`. No-manifest mode: degrades to `etai`. |
| `eta build [--profile <release\|debug>] [--bin NAME]` | S3 | manifest | Writes `.eta/target/<profile>/*.etac`; default profile is `release` (`-O --no-debug`); lockfile read-only in S3/S4. |
...
- Profile policy is explicit: package builds default to `release`
  (`etac -O --no-debug`) for runtime speed; debugger entry points use
  the `debug` profile (debug spans enabled).
- This is a contract-level decision in the plan; editor/debugger wiring
  lands in S7 and is not a blocker for S0-S6.
...
- Scheduling note: this behavior is implemented in S7 (Tooling
  integration). Earlier stages only lock the profile contract.
- VS Code debug sessions default to the interpreter path (`etai` /
  source mode), preserving debug spans (equivalent to not passing
  `--no-debug`).
...
- Release artifacts built via the default `release` profile use
  `-O --no-debug`; they are for runtime execution, not primary
...
2. **Examples.** Add a canonical package fixture at
   `packages/example/hello-world/` (`eta.toml`, `src/hello_world.eta`,
   `tests/smoke.test.eta`). In parallel, each `cookbook/<topic>/` gets
   an `eta.toml` with a single `path` dep on the in-tree stdlib. The current
...
3. **Tests.** `stdlib/tests/*.test.eta` are reorganised under the new
   ...
   programmatic API.
4. **CI.** Add a `eta build --profile release` step that exercises the
...
| Cross-platform | CI matrix | Windows / Linux / macOS runs `eta build` on `packages/example/hello-world` and asserts byte-identical `.etac` |
...
- Migrate selected examples to manifests, including
  `packages/example/hello-world` as the canonical smoke fixture (full
  cookbook migration can continue after S6).
...
- VS Code/DAP launch defaults are wired so debugging uses `profile=debug`
  while package build/run defaults remain `profile=release`.
...
$ eta build
   Compiling mathx v0.1.0 (../mathx)         [-O --no-debug …  3 fns]
   Compiling myapp v0.1.0 (.)                [-O --no-debug …  4 fns]
...
└── .eta/
    ...
    └── target/release/
        ├── mathx.etac                      ← -O, no debug, dep-hash table populated
        └── myapp.etac
```
