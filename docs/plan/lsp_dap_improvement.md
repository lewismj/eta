...
## Problem
Hover and completion documentation for special forms and builtins is currently
hardcoded in tool/session code instead of being derived from language/runtime
metadata.
...
Issues:
1. **Duplication / drift** — LSP hover, LSP completion, session hover, and
   session completion do not share one source of truth.
...
```cpp
namespace eta::docs {
std::string render_markdown(const DocEntry& entry);
std::string render_builtin_markdown(const eta::runtime::BuiltinMetadata& builtin);
} // namespace eta::docs
```
...
std::span<const eta::docs::DocEntry> special_form_docs();
std::optional<eta::docs::DocEntry> lookup_special_form_doc(std::string_view name);
} // namespace eta::reader
```
This removes special-form tables from both LSP and session code.
### Builtin metadata
...
```cpp
namespace eta::docs {
std::optional<DocEntry> lookup_language_doc(std::string_view name);
std::optional<DocEntry> doc_from_builtin_metadata(const eta::runtime::BuiltinMetadata& b);
std::string render_markdown(const DocEntry& entry);
std::string render_builtin_markdown(const eta::runtime::BuiltinMetadata& b);
} // namespace eta::docs
```
`lookup_language_doc(name)` checks special forms only. Builtins are resolved from
`eta::runtime::lookup_builtin_metadata(name)` so optional features track the
configured builtin metadata.
...
for (const auto& e : eta::reader::special_form_docs()) {
    ...
    items.push_back(json::object({
        ...
        {"kind", 14},
        {"detail", std::string(e.category.empty() ? "Keyword" : e.category)},
...
* Add `eta/core/src/eta/reader/special_form_docs.h` with the special-form docs
  currently hardcoded in LSP hover/completion.
* Add `eta/core/src/eta/runtime/builtin_metadata.h` with the builtin names,
  ...
  across `builtin_names.h` and LSP completion/hover tables.
...
* Add validation that runtime primitive registration order/name/arity/rest flags
  match `builtin_metadata()`.
...
* `nng_metadata_matches_configured_build` when NNG is enabled/disabled in CI
...
* Add DAP QA tests for known special form, known builtin, and unknown symbol.
...
* No hardcoded special-form or builtin symbol metadata tables remain under
  ...
  builtin categories, signatures, arities, and optional-feature lists.
* `eta/tools/lsp/` contains no `#ifdef ETA_HAS_NNG` used for symbol metadata.
* `eta/core/src/eta/runtime/builtin_names.h::register_builtin_names()` is driven
  by `builtin_metadata()`.
* LSP hover and completion use the same special-form and builtin metadata as
  session hover/completion.
* DAP docs are exposed through `eta/symbolDoc` or Eta-specific fields; standard
  DAP variable `"value"` remains runtime value only.
...
* QA tests for duplicate doc entries, builtin documentation coverage, LSP hover,
  LSP completion, session hover, and DAP `eta/symbolDoc` pass.
## Risks / non-goals
