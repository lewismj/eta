...
[â† Back to README](../README.md) Â· [Architecture](architecture.md) Â·
[NaN-Boxing](nanboxing.md) Â· [Bytecode & VM](bytecode-vm.md) Â·
[Runtime & GC](runtime.md) Â· [Networking](networking.md) Â·
...
**Key sources:**
[`module_linker.h`](../eta/core/src/eta/reader/module_linker.h) Â·
[`expander.h`](../eta/core/src/eta/reader/expander.h) Â·
...
```scheme
(import std.prelude)  ;; imports the stable prelude surface
```
...
| `display->string` | `(x) â†’ string` | Render value to string via string port |
| `read-line` | `([port]) â†’ string \| #f` | Read a line (returns `#f` at EOF) |
| `with-port` | `(port thunk) â†’ result` | Convenience wrapper for `with-output-to-port` |
| `with-input-port` | `(port thunk) â†’ result` | Convenience wrapper for `with-input-from-port` |
...
| `print` | `(x)` | Deprecated alias of `display` |
| `display-to-string` | `(x)` | Deprecated alias of `display->string` |
...
### `std.db` â€” Relation Database & Tabling
...
| Category | Key Functions |
|----------|---------------|
| **Definition** | `defrel`, `defrel-clause` |
| **Mutation** | `assert-fact!`, `retract-fact!`, `retract-all` |
| **Calls** | `call-rel`, `call-rel?` |
| **Indexing / tabling** | `index-rel!`, `tabled`, `tabled-clause` |
| **Deprecated aliases** | `assert`, `retract` |

> **Full documentation:** [Relations & Tabling](db.md)
...
Re-exports the stable public names from `std.core`, `std.math`, `std.io`,
`std.collections`, `std.logic`, `std.clp`, `std.clpb`, `std.clpr`,
`std.causal`, `std.fact_table`, `std.db`, `std.stats`, `std.net`, and
`std.test` in a single import for convenience.
