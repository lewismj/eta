...
[← Stdlib Reference](./README.md) · [Language Guide](../../language_guide.md) · [Detailed reference](../reference/log.md)
...
Each call uses the default logger. Optionally pass a logger as the first
argument to target a specific logger.

Level ordering: `trace < debug < info < warn < error < critical < off`.
Levels are plain symbols: `'trace`, `'debug`, `'info`, `'warn`, `'error`,
`'critical`, `'off`.
...
| `(log:set-formatter! logger formatter)` | Set a custom formatter (`'human` or `'json`). |
...
> **See also:** The [detailed reference](../reference/log.md) includes full
> sink option syntax, formatter modes, flush policies, and test recipes.

---

**Source:** [`stdlib/std/log.eta`](../../../stdlib/std/log.eta)
