...
const disassemblyView_1 = require("../../src/disassemblyView");
const disassemblyTreeView_1 = require("../../src/disassemblyTreeView");
const SAMPLE = [
    '; 2 function(s)',
    '',
    '=== outer ===',
...
describe('disassembly parser (B4)', () => {
    it('groups by function header', () => {
        const funcs = (0, disassemblyTreeView_1.parseFunctions)(SAMPLE, /* currentPC */ 1);
    ...
    it('marks the PC line and tags Call lines with their callee index', () => {
        const funcs = (0, disassemblyTreeView_1.parseFunctions)(SAMPLE, 1);
    ...
    it('findPcLine resolves the buffer line of the current PC', () => {
        const line = (0, disassemblyView_1.findPcLine)(SAMPLE, 1, 'outer');
        assert.ok(line >= 0);
        assert.match(SAMPLE.split('\n')[line], /^\s*1:\s+Call/);
    });
    it('findFunctionHeaderLine indexes by registry order', () => {
        const lines = SAMPLE.split('\n');
        const outerLine = (0, disassemblyView_1.findFunctionHeaderLine)(SAMPLE, 0);
        const innerLine = (0, disassemblyView_1.findFunctionHeaderLine)(SAMPLE, 1);
        assert.strictEqual(lines[outerLine], '=== outer ===');
        assert.strictEqual(lines[innerLine], '=== inner ===');
        assert.strictEqual((0, disassemblyView_1.findFunctionHeaderLine)(SAMPLE, 2), -1);
    });
    it('inferCalleeFuncIndex walks back to the nearest LoadConst <func:N>', () => {
        const lines = SAMPLE.split('\n');
        const callLineIdx = lines.findIndex((l) => /Call\b/.test(l));
        assert.strictEqual((0, disassemblyView_1.inferCalleeFuncIndex)(SAMPLE, callLineIdx), 1);
        // Non-Call line returns undefined.
        const returnIdx = lines.findIndex((l) => /Return\b/.test(l));
        assert.strictEqual((0, disassemblyView_1.inferCalleeFuncIndex)(SAMPLE, returnIdx), undefined);
        // Crossing a function boundary returns undefined.
        const innerCall = lines.findIndex((l, i) => i > callLineIdx && /Call\b/.test(l));
        if (innerCall >= 0) {
            assert.strictEqual((0, disassemblyView_1.inferCalleeFuncIndex)(SAMPLE, innerCall), undefined);
        }
    });
});
//# sourceMappingURL=disasm.test.js.map